...
while true; do
    if docker ps | grep -q "my_rustdesk_project-rustdesk-1"; then
        ...
        ip_result=$(stdbuf -oL sudo ip -4 addr show scope global | grep -oP 'inet \K[\d.]+' | head -n 1 | sed 's/^/访问地址: /' | sed 's/$/:\033[0m21114/' | sed 's/访问地址:/访问地址:\033[32m/')
        ip_result=${ip_result//$'\r'/}
        echo -e "$ip_result"
